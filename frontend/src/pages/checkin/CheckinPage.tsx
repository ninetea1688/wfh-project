  ...
  const checkInMutation = useMutation({
    mutationFn: async (data: FormData) => {
      const res = await attendanceService.checkIn({
        ...
        locationName: gps.latitude ? `${gps.latitude.toFixed(4)}° N, ${gps.longitude?.toFixed(4)}° E` : undefined,
      })
    ...
    return (
      <StaffLayout>
        <div className="p-6 text-center">
          ...
          <h2 className="text-lg font-semibold text-slate-700 mb-2">ลงชื่อแล้ววันนี้</h2>
          <p className="text-sm text-slate-500 mb-4">
            เข้างาน {formatTime(todayData.checkInTime)} · {todayData.workType === 'WFH' ? '🏠 WFH' : '🚗 ออกราชการ'}
          </p>
          <button
    ...
    <StaffLayout>
      <div className="bg-navy px-4 py-3 pb-5">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-white text-sm font-medium">ลงชื่อปฏิบัติงาน</span>
        </div>
        <p className="text-white/60 text-xs">{formatThaiDate(new Date())} · {formatTime(new Date())}</p>
      </div>

      <form onSubmit={handleSubmit((d) => checkInMutation.mutate(d))} className="pb-6">
        {/* Time card */}
        <div className="mx-4 -mt-3 bg-white rounded-xl shadow-sm border border-slate-100 p-4 mb-4">
          <div className="flex justify-between items-start mb-3">
            <div>
              ...
              <p className="text-2xl font-semibold text-slate-800">{formatTime(new Date())}</p>
            </div>
        ...
        {/* Work type */}
        <p className="text-xs font-medium text-slate-700 mx-4 mb-2">ประเภทการปฏิบัติงาน</p>
        <div className="grid grid-cols-2 gap-3 mx-4 mb-4">
          {(['WFH', 'FIELD'] as const).map((type) => (
            ...
            >
              ...
              <div className={`text-xs font-medium ${workType === type ? 'text-navy' : 'text-slate-500'}`}>
                {type === 'WFH' ? 'Work From Home' : 'ออกปฏิบัติราชการ'}
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="mx-4 mb-4 bg-white rounded-xl border border-slate-100 p-3 flex items-center gap-2">
          <MapPin size={14} className={gps.latitude ? 'text-success' : 'text-slate-400'} />
          {gps.loading ? (
            ...
            <span className="text-xs text-slate-400 flex-1">{gps.error ?? 'ไม่พบพิกัด GPS'}</span>
          )}
          {gps.latitude && <span className="text-xs text-success font-medium">ตรวจพบ GPS</span>}
        </div>
        ...
        {errors.taskDescription && (
          <p className="text-xs text-danger mx-4 mb-3">{errors.taskDescription.message}</p>
        )}
        ...
        <div className="mx-4 mb-4">
          ...
          {selectedFiles.length < MAX_FILES && (
            ...
            >
              ...
              <p className="text-xs text-slate-400">แนบรูปภาพหลักฐาน (ไม่เกิน {MAX_FILES} รูป)</p>
              <p className="text-[10px] text-slate-300 mt-1">JPEG / PNG · ไม่เกิน 5MB</p>
            </button>
          )}
