...
type WorkType = "WFH" | "OFFICE" | "FIELD" | "ON_SITE";
...
> = {
  ...
  OFFICE: { label: "สำนักงาน",        bg: "bg-green-100",  text: "text-green-800"  },
  FIELD:  { label: "ไปราชการ",        bg: "bg-amber-100",  text: "text-amber-800"  },
  LEAVE:  { label: "ลา",              bg: "bg-red-100",    text: "text-red-800"    },
  ON_SITE: { label: "ออกพื้นที่",    bg: "bg-purple-100", text: "text-purple-800" },
};
...
> = {
  WFH:    { label: "WFH",          bg: "bg-blue-100",   text: "text-blue-800",   dot: "bg-blue-500"   },
  OFFICE: { label: "สำนักงาน",     bg: "bg-green-100",  text: "text-green-800",  dot: "bg-green-500"  },
  FIELD:  { label: "ไปราชการ",     bg: "bg-amber-100",  text: "text-amber-800",  dot: "bg-amber-500"  },
  ON_SITE: { label: "ออกพื้นที่", bg: "bg-purple-100", text: "text-purple-800", dot: "bg-purple-500" },
};
...
function exportDayToExcel(dateStr: string, entries: AdminDayResultEntry[]) {
  const rows = entries.map((e, i) => ({
    ลำดับ: i + 1,
    ชื่อ: `${e.firstName} ${e.lastName}`,
    ตำแหน่ง: e.position ?? "—",
    หน่วยงาน: e.department?.name ?? "—",
    แผนงาน: e.planType ? `${PLAN_CONFIG[e.planType as PlanType]?.label ?? e.planType}${e.planNote ? `-(${e.planNote})` : ""}` : "—",
    ผลงาน: WORK_CONFIG[e.workType as WorkType]?.label ?? e.workType,
    สถานะ: STATUS_CONFIG[e.status]?.label ?? e.status,
    เข้างาน: formatTime(e.checkInTime),
    ออกงาน: formatTime(e.checkOutTime),
    บันทึก: e.taskDescription ?? "",
  }));
  const ws = XLSX.utils.json_to_sheet(rows);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "ผลการปฏิบัติงาน");
  XLSX.writeFile(wb, `ผลงาน_${dateStr}.xlsx`);
}
...
function exportMonthToExcel(
  ...
  const ws = XLSX.utils.json_to_sheet(rows);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "ผลการปฏิบัติงานรายเดือน");
  XLSX.writeFile(wb, `ผลงาน_${format(currentMonth, "yyyy-MM")}.xlsx`);
}
...
export default function AdminResultsCalendarPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      workplanService.getAdminMonthResults(year, month).then((r) => r.data.data),
  });
  ...
  let d = gridStart;
  while (d <= gridEnd) { calendarDays.push(d); d = addDays(d, 1); }
  ...
  const tableDays = eachDayOfInterval({ start: monthStart, end: monthEnd }).filter(
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">สรุปผลการลงชื่อปฏิบัติงานของบุคลากรทั้งหมด รายเดือน</p>
          </div>
          <div className="flex items-center gap-3 flex-wrap justify-end">
            {(Object.entries(WORK_CONFIG) as [WorkType, typeof WORK_CONFIG.WFH][]).map(([type, cfg]) => (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="space-y-0.5">
                          {(Object.entries(counts) as [WorkType, number][]).map(([type, count]) => (
                            <div key={type} className={`flex items-center gap-1 px-1 py-0.5 rounded text-[10px] font-medium ${WORK_CONFIG[type].bg} ${WORK_CONFIG[type].text}`}>
                              <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${WORK_CONFIG[type].dot}`} />
                              {WORK_CONFIG[type].label} {count}
                            </div>
                          ))}
                          {entries.length > 0 && (
                            <p className="text-[10px] text-gray-400 pl-1">รวม {entries.length} คน</p>
                          )}
                        </div>
                      </button>
                    );
            ...
            {selectedDate && (
              <div className="w-80 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden self-start">
                ...
                {selectedEntries.length === 0 ? (
                  <div className="px-4 py-8 text-center text-gray-400 text-sm">ไม่มีข้อมูลการลงชื่อในวันนี้</div>
                ) : (
                  <>
                    <div className="px-4 py-2 border-b border-gray-100 flex gap-2 flex-wrap">
                      {(Object.entries(countByType(selectedEntries)) as [WorkType, number][]).map(([type, count]) => (
                        <span key={type} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${WORK_CONFIG[type].bg} ${WORK_CONFIG[type].text}`}>
                          {WORK_CONFIG[type].label}: {count}
                        </span>
                      ))}
                    </div>
                    <div className="divide-y divide-gray-50 max-h-[520px] overflow-y-auto">
                      {selectedEntries.map((entry) => {
                        const cfg = WORK_CONFIG[entry.workType as WorkType] ?? WORK_CONFIG.WFH;
                        const statusCfg = STATUS_CONFIG[entry.status] ?? STATUS_CONFIG.CHECKED_IN;
                        return (
                          <div key={entry.userId} className="px-4 py-2.5">
                            <div className="flex items-center gap-2 mb-1">
                              <span className={`w-2 h-2 rounded-full flex-shrink-0 ${cfg.dot}`} />
                              <p className="text-sm font-medium text-gray-800 flex-1 truncate">{entry.firstName} {entry.lastName}</p>
                              <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full flex-shrink-0 ${statusCfg.bg} ${statusCfg.text}`}>{statusCfg.label}</span>
                            </div>
                            <div className="pl-4 flex items-center gap-3 text-xs text-gray-500">
                              <span>{entry.department?.name ?? "—"}</span>
                              <span className={`px-1.5 py-0.5 rounded ${cfg.bg} ${cfg.text} font-medium`}>{cfg.label}</span>
                            </div>
                            <div className="pl-4 mt-1 flex items-center gap-2 text-xs text-gray-400">
                              <span>เข้า {formatTime(entry.checkInTime)}</span>
                              {entry.checkOutTime && <span>ออก {formatTime(entry.checkOutTime)}</span>}
                            </div>
                            {entry.taskDescription && (
                              <p className="pl-4 mt-0.5 text-xs text-gray-500 line-clamp-1">{entry.taskDescription}</p>
                            )}
                          </div>
                        );
                      })}
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                      {["วันที่", "ชื่อ-สกุล", "ตำแหน่ง", "หน่วยงาน", "แผนงาน", "ผลงาน", "สถานะ", "เข้างาน", "ออกงาน", "บันทึก", ""].map((h) => (
                        ...
                        return (
                          <tr key={`${dateStr}-${entry.userId}`} className="border-t border-slate-50 hover:bg-slate-50">
                            ...
                            <td className="px-3 py-2.5 max-w-[160px]">
                              {entry.planType ? (() => {
                                const pcfg = PLAN_CONFIG[entry.planType as PlanType];
                                return pcfg ? (
                                  <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium ${pcfg.bg} ${pcfg.text}`}>
                                    {pcfg.label}{entry.planNote ? <span className="font-normal opacity-75 truncate max-w-[80px]" title={entry.planNote}>({entry.planNote})</span> : null}
                                  </span>
                            ...
                            <td className="px-3 py-2.5">
                              <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${statusCfg.bg} ${statusCfg.text}`}>{statusCfg.label}</span>
                            </td>
                            <td className="px-3 py-2.5 text-slate-500 whitespace-nowrap">{formatTime(entry.checkInTime)}</td>
                            <td className="px-3 py-2.5 text-slate-500 whitespace-nowrap">{formatTime(entry.checkOutTime)}</td>
                            <td className="px-3 py-2.5 text-slate-500 max-w-[160px] truncate">{entry.taskDescription ?? "—"}</td>
                            {i === 0 ? (
