...
export const attendanceService = {
  checkIn: (payload: CheckInPayload) =>
    api.post<{ success: boolean; data: AttendanceRecord }>(
      "/attendance/checkin",
      payload,
    ),

  checkOut: () =>
    api.patch<{ success: boolean; data: AttendanceRecord }>(
      "/attendance/checkout",
    ),

  getToday: () =>
    api.get<{ success: boolean; data: AttendanceRecord | null }>(
      "/attendance/today",
    ),
  ...
  getHistory: (params?: {
    ...
    api.get<{
      success: boolean;
      data: { records: AttendanceRecord[]; total: number; totalPages: number };
